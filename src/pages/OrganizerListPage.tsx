import { ArrowRight, Calendar, LayoutDashboard, Loader2, Plus } from 'lucide-react';
			...
			<div className="mb-8 flex items-center justify-between">
				<div>
					...
					<p className="text-slate-500 text-sm mt-1">
						Manage your active and upcoming scavenger hunts.
					</p>
				</div>
				<Link
					to="/organizer/new"
			...
			{races.length === 0 ? (
				...
				<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
					{races.map((race) => (
						<div
							key={race.id}
							className="group flex flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition-all hover:shadow-md hover:border-indigo-100"
						>
							...
							<div className="border-t border-slate-50 bg-slate-50/50 p-4">
								<Link
									to={
										race.status === 'active'
											? `/organizer/dashboard/${race.id}`
											: `/organizer/lobby/${race.id}`
									}
