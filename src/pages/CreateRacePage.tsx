...
import { Flag, MapPin, Navigation, Save, Trash2 } from 'lucide-react';
				...
				<div className="flex-1 overflow-y-auto p-6 space-y-6">
					...
					<div className="space-y-4 text-sm">
						...
						{waypoints.map((wp, index) => (
							<div
								key={index}
								className="group relative rounded-2xl border border-slate-100 bg-slate-50 p-4 transition-all hover:bg-white hover:shadow-md"
							>
								...
								<div className="space-y-3">
									...
									<div>
										...
										<input
											...
											max={500}
											onChange={(e) =>
												updateWaypoint(index, { radius_meters: parseInt(e.target.value, 10) || 50 })
											}
										/>
									</div>
									<div>
										<label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-400">
											Hint (OPTIONAL, COSTS 10 PTS)
										</label>
										<input
											type="text"
											className="w-full rounded-lg border border-slate-200 bg-white p-2 text-xs focus:border-indigo-400 outline-none"
