...
export const TeamGameView: React.FC = () => {
	...
	const loadWaypoint = useCallback(
		async (rId: string, index: number) => {
	...
	useEffect(() => {
		...
		initialLoad();
	}, [raceId, loadWaypoint]); // Only run once on mount
	...
	}, [team?.current_step_index, raceId, loadWaypoint, team]);
	...
	const handleVerifyLocation = () => {
		...
		navigator.geolocation.getCurrentPosition(
			async (pos) => {
				...
				if (accuracy > 50) {
					setError(
						`Location accuracy too low (${Math.round(accuracy)}m). Try moving to a clearer area.`,
					);
					setVerifying(false);
					return;
				}
	...
	if (viewState === 'finished') {
		return (
			<div className="mx-auto max-w-md py-12 text-center px-4 animate-in fade-in zoom-in duration-700">
				...
				<h1 className="text-4xl font-black text-slate-900 mb-2 mt-8 uppercase tracking-tight">
					Race Finished!
				</h1>
				<p className="text-slate-500 mb-10 text-lg font-medium">
					You completed all waypoints. Check the leaderboard for your final rank!
				</p>
					...
					<div className="mt-6 border-t border-slate-50 pt-4">
						{hintRevealed ? (
							<div className="rounded-xl bg-amber-50 p-4 text-xs font-medium text-amber-800 border border-amber-100 animate-in zoom-in-95 duration-300">
								<span className="font-bold text-amber-600 uppercase tracking-widest block mb-1 text-[10px]">
									The Hint:
								</span>
								{waypoint.hint}
							</div>
			...
			<div className="space-y-4">
				...
				>
					{verifying ? (
						...
						<>
							<MapPin className="h-6 w-6" /> I'M HERE
						</>
					)}
				</button>
							...
							<>
								<div className="mb-4 flex items-center gap-3">
									...
									<div>
										<p className="text-sm font-bold text-slate-900">
											{submissionStatus === 'rejected' ? 'Re-upload Photo' : 'Photo Task'}
										</p>
										<p className="text-xs text-slate-500">{waypoint?.task_instruction}</p>
									</div>
								</div>
								{submissionStatus === 'rejected' && (
									<div className="mb-4 rounded-xl bg-rose-50 p-3 text-xs text-rose-600 border border-rose-100 italic">
								...
								<label className="block">
									<input
										...
										className="block w-full text-sm text-slate-500 file:mr-4 file:rounded-xl file:border-0 file:bg-indigo-600 file:px-6 file:py-3 file:text-sm file:font-semibold file:text-white hover:file:bg-indigo-700 disabled:opacity-50"
									/>
								</label>
							</>
						) : (
