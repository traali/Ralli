...
import { Activity, MapPin, Camera, CheckCircle2, AlertCircle, Clock } from 'lucide-react';
...
export const ActivityFeed: React.FC<{ raceId: string }> = ({ raceId }) => {
    ...
    const getIcon = (type: string) => {
        switch (type) {
            case 'verify': return <MapPin className="h-3 w-3 text-indigo-500" />;
            case 'submit': return <Camera className="h-3 w-3 text-amber-500" />;
            case 'approve': return <CheckCircle2 className="h-3 w-3 text-emerald-500" />;
            case 'reject': return <AlertCircle className="h-3 w-3 text-rose-500" />;
