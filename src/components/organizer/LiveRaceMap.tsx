...
export const LiveRaceMap: React.FC<{ raceId: string }> = ({ raceId }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const { data: wpData } = await supabase.from('waypoints').select('*').eq('race_id', raceId).order('order_index');
        ...
        const channel = supabase
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'teams', filter: `race_id=eq.${raceId}` }, () => {
                fetchData();
            })
            .subscribe();
                                    ...
                                    <button
                                        onClick={async (e) => {
                                            ...
                                            if (confirm(`Force unlock next step for ${team.name}?`)) {
                                                await supabase.from('teams').update({ current_step_index: team.current_step_index + 1 }).eq('id', team.id);
                                            }
                                        }}
